...
def _gomulu_pluginleri_kaydet(): pass
...
def t(anahtar, dil=""): return anahtar
...
def rol_kontrol(gereken_rol): return True
...
# ══════════════════════════════════════════════════════════════════
# DUMMY FONKSIYONLAR (NameError Hatalarını Engellemek İçin Eklendi)
# ══════════════════════════════════════════════════════════════════
def aktivite_logu_metin(): return "Aktivite kaydı yok."
def dashboard_html(): return "<div style='color:white; padding:10px;'>Analitik verisi yok.</div>"
def plugin_yeniden_yukle(): return "Pluginler güncellendi."
def plugin_pasif_yap(isim): return f"Pasif: {isim}"
def plugin_aktif_yap(isim): return f"Aktif: {isim}"
def webhook_test_gonder(id): return f"Test edildi: {id}"
def webhook_toggle(id): return f"Durum değişti: {id}"
def hatirlatici_sil(id): return f"Silindi: {id}"
def kullanici_rol_degistir(isim, rol): return f"Rol değişti."
def kullanici_sil(isim): return f"Silindi: {isim}"
def ortak_not_ekle(metin): return "Not eklendi."
def duyuru_ekle(metin): return "Duyuru yayınlandı."
...
''')
            ...
            hizli_temiz = gr.Button("🧹  Sohbeti Temizle", variant="secondary", elem_classes=["lbtn"])
    ...
    # Plugin eventleri
    pl_yenile.click(fn=plugin_yeniden_yukle, outputs=[pl_durum]).then(fn=lambda: "Plugin Listesi", outputs=[pl_liste])
    pl_pasif_b.click(fn=plugin_pasif_yap, inputs=[pl_isim_in], outputs=[pl_yonetim_d]).then(fn=lambda: "Plugin Listesi", outputs=[pl_liste])
    pl_aktif_b.click(fn=plugin_aktif_yap, inputs=[pl_isim_in], outputs=[pl_yonetim_d]).then(fn=lambda: "Plugin Listesi", outputs=[pl_liste])
